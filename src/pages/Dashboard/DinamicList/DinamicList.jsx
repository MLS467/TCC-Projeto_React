...
const DinamicList = () => {
  ...
  return (
    <>
      {dashboradContext.data?.data.length > 0 ? (
        <TableWrapperList>
            ...
            <TableBodyList>
              {dashboradContext.data?.data.map((item, index) => (
                <TableRowList key={index}>
                  ...
                  <ContainerDashboard>
                    <Link to={`/dashboard/edit_funcionario/${tipo}/${item.id}`}>
                      <CiEdit className="btnDash btn_edit" />
                    </Link>
                    <CiTrash
                      className="btnDash btn_delete"
                      onClick={() =>
                        dashboradContext.requestDelete(item.id, item.user.role)
                      }
                    />
                  </ContainerDashboard>
                </TableRowList>
